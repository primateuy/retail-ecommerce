...
/**
 * Escucha el bus tras un cobro OCA async desde account.payment: aviso al usuario
 * y soft_reload del formulario si sigue abierto el mismo registro (sin F5 manual).
 */
export const accountPaymentOCABusService = {
    dependencies: ["bus_service", "action", "notification", "router"],

    start(env, { bus_service, action, notification, router }) {
        bus_service.subscribe("oca_account.payment_updated", async (payload) => {
...
registry.category("services").add("account_payment_oca_bus", accountPaymentOCABusService);
