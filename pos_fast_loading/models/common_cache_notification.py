...
PRODUCTFIELD = ['display_name', 'list_price', 'lst_price', 'standard_price', 'categ_id', 'pos_categ_ids', 'taxes_id', 'barcode', 'default_code', 'to_weight', 'uom_id', 'description_sale', 'description', 'product_tmpl_id', 'tracking', 'active', 'available_in_pos','optional_product_ids', 'attribute_line_ids','combo_ids', 'image_128']
...
IMAGEFIELDS = ['image_1024', 'image_1920', 'image_256', 'image_512', 'image_variant_1024',
...
class CommonCacheNotification(models.Model):
    ...
    # @get_product_fields.setter
    # def get_product_fields(self, vals):
    ...
    # @get_partner_fields.setter
    # def get_partner_fields(self, vals):
    #     list(set(PARTNERFIELD + [str(data.name) for data in vals]))
    ...
    def _update_cache_file(self, bin_file, Updated_conv_data):
        ...
        updated_data = base64.encodebytes(json.dumps(json_data, default=date_utils.json_default).encode('utf-8'))
        ...
        if mongo_server_rec:
            ...
            if mongo_server_rec.partner_all_fields:
                if load_pos_data_type == 'postgres':
                    ...
                    #     JOIN res_partner p ON f.model = 'res.partner' AND p.company_id = %s
                    #     WHERE f.ttype NOT IN ('binary', 'properties', 'properties_definition')
                    # """, (self.env.company.id,))
            ...
            if mongo_server_rec.product_all_fields:
                if load_pos_data_type == 'postgres':
                    ...
                    #     WHERE model = %s AND ttype NOT IN ('binary', 'properties', 'properties_definition')
                    # """, ('product.product',))
