# -*- coding: utf-8 -*-
"""
Integración Fiserv ITD en el punto de venta (pos.payment.method).

Los helpers HTTP, constantes (FISERV_ITD_RESPONSE_CODE_MSG) y el worker de hilo
contable (fiserv_itd_background_worker_account_payment) viven en
``odoo_pos_fiserv_core.models.fiserv_utils``; aquí se importan y se usan.
"""
...
from odoo.addons.odoo_pos_fiserv_core.models.fiserv_utils import (
    ...
    _fiserv_extract_itd_message_from_body,
    _fiserv_http_post_confirm_financial_purchase,
    _fiserv_normalize_itd_http_response,
    _fiserv_resolve_quota_for_confirm,
...
class PosPaymentMethod(models.Model):
    ...
    def _fiserv_http_post_refund(self, data):
        """POST a processFinancialPurchaseRefund (devolución tras cierre de lote)."""
        self.ensure_one()
        base_url_endpoint = self._fiserv_itd_base_url_webservice()
        return fiserv_itd_http_post(
            base_url_endpoint,
    ...
    def processFinancialPurchaseVoidByTicket(self, data, pos_session_id, account_payment_id=None):
        ...
        Si ITD rechaza el void con RC 109/110 (ticket fuera del lote actual —hubo
        cierre de lote entre la venta y el intento de anular), se hace fallback
        automático a ``processFinancialPurchaseRefund`` con los datos tomados de
        la transacción Fiserv original (buscada por ``TicketNumber``).
        """
        self.ensure_one()
        ...
        if rc in FISERV_ITD_RC_VOID_SHOULD_REFUND:
            _logger.info(
                'Fiserv POS void rechazó RC=%s (ticket fuera del lote actual). '
                'Fallback automático a processFinancialPurchaseRefund. ticket=%s',
                rc, data.get('TicketNumber'),
            )
            data, base_url_endpoint, response_json = self._fiserv_switch_void_to_refund(
                data, account_payment_id=account_payment_id,
            )
            rc = str(response_json.get('ResponseCode', '999')).strip()

        if rc != '0':
            # Error inicial: persistir la transacción como error para auditoría.
            self._store_fiserv_transaction(data, response_json, pos_session_id or False)
            return response_json
    ...
    def _fiserv_switch_void_to_refund(self, void_data, account_payment_id=None):
        """
        Tras un void rechazado por RC 109/110, construye el payload de refund
        desde la ``payment.transaction`` original (buscada por TicketNumber) y
        dispara ``processFinancialPurchaseRefund``.

        El monto del refund es el monto total de la transacción original; si en
        el futuro se necesita refund parcial, el llamador puede pasar el monto.

        Returns:
            tuple: (refund_data, base_url_endpoint, response_json)
        """
        self.ensure_one()
        ticket = (void_data.get('TicketNumber') or '').strip()
        if not ticket:
            raise UserError(_('Falta TicketNumber; no se puede iniciar devolución.'))
        source_tx = self.env['payment.transaction'].sudo().search(
            [
        ...
        if not source_tx:
            raise UserError(
                _('No se encontró la transacción Fiserv original (ticket %s) para devolución.') % ticket
            )
        provider = self.fiserv_provider_id or source_tx.provider_id
        if not provider or provider.code != 'fiserv':
            raise UserError(
                _('El método de pago POS no tiene proveedor Fiserv asociado; no se puede hacer devolución.')
            )
        amount_cents = int(round((source_tx.amount or 0) * 100))
        if amount_cents <= 0:
            raise UserError(_('Monto inválido en la transacción original; no se puede devolver.'))
        refund_data = provider._prepare_fiserv_itd_refund_payload(
            source_transaction=source_tx,
            pos_id=str(void_data.get('PosID', '')),
            amount_cents=amount_cents,
            user_id=self.env.user.id,
        )
        _logger.info('Fiserv POS processFinancialPurchaseRefund %s', pprint.pformat(refund_data))
        base_url_endpoint, response_json = self._fiserv_http_post_refund(refund_data)
        return refund_data, base_url_endpoint, response_json
