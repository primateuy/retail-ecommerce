...
class PosOrder(models.Model):
    ...
    def _generate_pos_order_invoice(self):
        """
        Si el pago ya fue capturado por el pinpad Fiserv (``payment.transaction``
        en ``done``), atrapar cualquier excepción del flujo de facturación para
        que la orden quede persistida como ``paid`` y el POS no se freeze.
        ...
        """
        self.ensure_one()
        has_fiserv_tx = bool(self.env['payment.transaction'].sudo().search_count([
            ('pos_order_id', '=', self.id),
            ('provider_id.code', '=', 'fiserv'),
            ('state', '=', 'done'),
        ]))
        if not has_fiserv_tx:
            return super()._generate_pos_order_invoice()
        try:
            return super()._generate_pos_order_invoice()
        except Exception as exc:
            _logger.error(
                "POS Fiserv: fallo generando factura orden %s; orden queda paid "
            ...
            try:
                ...
                _logger.debug("POS Fiserv: no se pudo setear to_invoice=False", exc_info=True)
            try:
                ...
                _logger.debug("POS Fiserv: no se pudo postear mensaje en chatter", exc_info=True)
            return False
