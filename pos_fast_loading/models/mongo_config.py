...
class MongoServerConfig(models.Model):
    ...
    def get_data_on_sync(self, **kwargs):
        ...
        if mongo_server_rec:
            ...
            try:
                self.env['common.cache.notification'].with_context(coming_from_pos=True).get_common_changes()
                new_cache_records = self.env['common.cache.notification'].search(
                    [('state', '!=', 'done'),('config_id', '=', config_id.id)])
                ...
                if new_cache_records:
                    ...
                    if load_pos_data_type == 'mongo':
                        ...
                        if len(product_record_ids) and config_id:
                            product_res = self.env['product.product'].with_user(self.env.context['uid']).with_company(
                                config_id.company_id.id).search_read([('id', 'in', product_record_ids)], fields=self.return_fields(mongo_server_rec, 'product.product', 'product_field_ids', PRODUCTFIELD, 'product_all_fields'))
    ...
    def load_rem_product(self, page, company_id, session_id):
        ...
        if binary_data_rec and mongo_server_rec and mongo_server_rec.load_pos_data_from == 'postgres':
            ...
            if(products):
                ...
                session._process_pos_ui_product_product(products)
    ...
    def create_partners_batch(self, mongo_server_rec, server_data_batch, partner_fields, company):
        ...
        while (offset < partner_len):
            ...
            data = {'server_data_cache': base64.encodebytes(json.dumps(partner_data, default=date_utils.json_default).encode('utf-8'))}
            data.update({
    ...
    def create_product_batch(self, mongo_server_rec, server_data_batch, product_fields, company):
        ...
        while (offset < product_len):
            ...
            temp = search_data_len + offset if search_data_len < mongo_server_rec.limit else mongo_server_rec.limit+offset
            data = {'server_data_cache': base64.encodebytes(json.dumps(products_data, default=date_utils.json_default).encode('utf-8'))}
            data.update({
                'mongo_config': mongo_server_rec.id,
                'offset_start': offset,
                'min_id': min_id,
