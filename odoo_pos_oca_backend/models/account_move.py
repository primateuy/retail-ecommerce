...
pago (integración OCA) en lugar de pasar por el wizard estándar.
"""
...
class AccountMove(models.Model):
    ...
    # Si Fiserv backend también está instalado, ambos módulos añaden un botón
    # "Registrar pago" equivalente. Este compute sobrescribe al de Fiserv
    # (por el orden de carga de módulos) para que retorne True y se oculte el
    # botón Fiserv duplicado; el de OCA queda como único visible.
    ...
    def _compute_pos_register_hide_fiserv(self):
        # OCA siempre oculta el botón Fiserv cuando coexisten: la UX es
        # idéntica, basta con un botón custom visible.
        for rec in self:
            rec.pos_register_hide_fiserv = True

    def action_oca_register_payment(self):
        """
        ...
        cuando el diario elegido sea integrado OCA.
        """
        ...
        ctx = {
            ...
            'default_oca_source_invoice_ids': [(6, 0, invoices.ids)],
        }
        return {
